...
energy.ttest <- t.test(Dcalls_2hrBvsA_earthquakes.df$energy_B, Dcalls_2hrBvsA_earthquakes.df$energy_A, paired = TRUE)
...
# energy 
energy.lm <- lm(delta_energy ~ depth + magnitude + DistOrigin + Energy + ncalls_B + DOY +
                    factor(Channel), data = Dcalls_2hrBvsA_earthquakes.df)
summary(energy.lm)
...
energy_BvsA_pivot.df <- Dcalls_2hrBvsA_earthquakes.df %>% 
  pivot_longer(6:7,names_to = "BvsA") %>% #
  mutate(publicid_channel = as.factor(publicid_channel)) # Make factors 
energy_BvsA_pivot.df$BvsA <- factor(energy_BvsA_pivot.df$BvsA, levels=c("energy_B", "energy_A"))
# song intensity
...
# relative rl
Dcallrl.tsplot <- ggplot(data = calls_earthquakes_15min_2hr.df, aes(x = TimeBin, y = Dcall_rl.mean, group = group)) +
  geom_point(size = 2, color = "dark blue") + geom_line(color = "dark blue") +
  geom_errorbar(aes(ymin=Dcall_rl.mean-Dcall_rl.se, ymax=Dcall_rl.mean+Dcall_rl.se), width = 0.5, color = "dark blue") +
  geom_vline(xintercept = "t00a", color = "dark blue", linetype = "dashed", size = 0.75) +
  scale_x_discrete(labels = c("t-120", "t-105", "t-90", "t-75", "t-60", "t-45", "t-30", "t-15", "t+00","t+15", "t+30", "t+45", "t+60", "t+75", "t+90", "t+105")) +
  theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
...
EQrl.hist <- ggplot(data = EQspecs.df, aes(x=Energy)) +
  geom_histogram(color="dark blue", fill="dark blue", alpha=0.5) +
  scale_x_continuous(expand = c(0,0)) + scale_y_continuous(expand = c(0,0)) +
  ylab("Count") + xlab("Relative received level (dB)") +
  theme_bw()
...
# energy 
energy_1hr.lm <- lm(delta_energy ~ depth + magnitude + DistOrigin + Energy + ncalls_B + DOY +
                    factor(Channel), data = Dcalls_1hrBvsA_earthquakes.df)
...
# energy 
energy_2hr.lm <- lm(delta_energy ~ depth + magnitude + DistOrigin + Energy + ncalls_B + DOY +
                    factor(Channel), data = Dcalls_2hrBvsA_earthquakes.df)
...
# energy 
energy_3hr.lm <- lm(delta_energy ~ depth + magnitude + DistOrigin + Energy + ncalls_B + DOY +
                    factor(Channel), data = Dcalls_3hrBvsA_earthquakes.df)
...
## 4 hour
# n calls 
ncalls_4hr.lm <- lm(delta_ncalls ~ depth + magnitude + DistOrigin + Energy + ncalls_B + DOY +
